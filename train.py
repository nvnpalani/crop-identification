...
def train_model(dataset_path, upload_path, model_file, class_file, model_name):
    ...
    try:
        ...
        try:
            if os.path.exists("app_data/scenario_cache.json"):
                with open("app_data/scenario_cache.json", "r") as f:
                    cache = json.load(f)
                    model_key = "fruit"
                    if "mango" in model_name.lower(): model_key = "mango"
                    elif "leaf" in model_name.lower(): model_key = "leaf"
        ...
        if os.path.exists(checkpoint_file):
            print(f"\n[INFO] Found CHECKPOINT '{checkpoint_file}'. Resuming training...")
        ...
        print(f"==================================================")
        print(f" Crop Name       : {model_name.replace(' Model', '')}")
        ...
        history = model.fit(
            ...
            callbacks=[early_stop, lr_reduce, checkpoint_cb],
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        disease_type = sys.argv[2].lower()

    base_crop_dir = "crop_datasets"
    base_upload_dir = "user_uploads"

    if os.path.exists(base_crop_dir):
        crops_to_train = [crop_name] if crop_name != "all" else [d for d in os.listdir(base_crop_dir) if os.path.isdir(os.path.join(base_crop_dir, d))]
        
        for c_name in crops_to_train:
            c_path = os.path.join(base_crop_dir, c_name)
            if not os.path.isdir(c_path): continue
            
            types_to_train = [disease_type] if disease_type != "all" else [d for d in os.listdir(c_path) if os.path.isdir(os.path.join(c_path, d))]
            
            for d_type in types_to_train:
                dataset_path = os.path.join(c_path, d_type)
                upload_path = os.path.join(base_upload_dir, c_name, d_type)
                # Ensure model directory exists
                model_dir = os.path.join("models", c_name)
                os.makedirs(model_dir, exist_ok=True)
                
                # Standardized model and class file names
                model_file = os.path.join(model_dir, f"{d_type}_model.keras")
                class_file = os.path.join(model_dir, f"{d_type}_classes.json")
                model_name_log = f"{c_name.capitalize()} {d_type.replace('_', ' ').capitalize()} Model"
                    
                print(f"\n--- {c_name.upper()} : {d_type.upper()} Start Training ---")
                train_model(
